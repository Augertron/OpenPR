...
/************************************************************
 * convert SCI matrix to IplImage
************************************************************/
...
* change the data order from column-wise to row-wise
***********************************************************/ 
CvMat* SciMat2CvMat(int nPos)
{
	CvMat* pMat = NULL;
	int mR, nR, lR;
    int i, j, k;
    
    GetRhsVar(nPos, MATRIX_OF_DOUBLE_DATATYPE, &mR, &nR, &lR);
 // double *pSrc = stk(lR);
    
    pMat = cvCreateMat(mR, nR, CV_32FC1); //data type here is fixed
    k = 0;
    for(j = 0; j < nR; j++)
    {
    	for(i = 0; i < mR; i++)
    	{
    		pMat->data.fl[i*nR+j] = (float)stk(lR)[k++];//(float)pSrc[k++];
    	}
    }
